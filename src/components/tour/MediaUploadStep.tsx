...
export const MediaUploadStep: React.FC<MediaUploadStepProps> = ({
  ...
  const handleMediaAdd = (stopIndex: number, newMedia: Media[]) => {
  ...
  const handleMediaRemove = (stopIndex: number, mediaIndex: number) => {
    const updatedStops = [...stops];
    const currentMedia = updatedStops[stopIndex].media || [];
  ...
  const getStopValidation = (stopIndex: number) => {
  ...
  const getTotalAudioDuration = (stopIndex: number) => {
    const stop = stops[stopIndex];
    const audioFiles = stop.media?.filter(m => m.media_type === 'audio') || [];
    return audioFiles.reduce((total, audio) => total + (audio.duration_seconds || 0), 0);
  };

  const getRecommendedAudioTime = (stopIndex: number) => {
