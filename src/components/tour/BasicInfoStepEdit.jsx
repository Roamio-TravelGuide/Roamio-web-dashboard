...
import { UploadCloud, X, ImageIcon, Loader2, AlertTriangle } from 'lucide-react';
import { uploadtempcover , viewtempcover} from '../../api/tour/tourApi'; // Assuming these API functions exist
...
const ImagePreview = ({ 
  ...
  <div className="relative group">
    <img
      ...
      className="object-cover w-full rounded-lg shadow-sm aspect-video"
      onError={() => {
...
export const BasicInfoStepEdit = ({ tourData, onUpdate }) => {
  ...
  useEffect(() => {
    // Set preview image from cover_image.url if available
    if (tourData.cover_image?.url) {
  ...
  }, [tourData.cover_image, tourData.title, tourData.description]);
  ...
  try {
    ...
    // Get the full accessible URL from the API
    ...
    onUpdate({ 
      ...
      cover_image_id: uploadResponse.id
    });
    ...
    if (error.name !== 'AbortError') {
      ...
      setPreviewImage(tourData.cover_image?.url || null);
    }
  } finally {
...
}, [validateFile, handleError, onUpdate, tourData.id, tourData.cover_image]);
              ...
              <div className="p-5 space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
