...
import React, { useState, useEffect } from 'react';
import { Calendar, MapPin, Users, X, Search, Filter, ChevronDown, Check, ChevronLeft, ChevronRight, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';
...
const ModeratorDashboard = () => {
  ...
  const convertApiResponseToTour = (apiTour) => {
    return {
      ...
      location: apiTour.location || 'Unknown Location',
      guide: {
        id: apiTour.guide?.user?.id || apiTour.guide_id,
        name: apiTour.guide?.user?.name || 'Unknown Guide',
        avatar: apiTour.guide?.user?.profile_picture || 'https://images.pexels.com/photos/614810/pexels-photo-614810.jpeg?auto=compress&cs=tinysrgb&w=100&h=100&fit=crop'
      },
      price: apiTour.price,
      status: apiTour.status,
      image: apiTour.cover_image || 'https://images.pexels.com/photos/1320684/pexels-photo-1320684.jpeg?auto=compress&cs=tinysrgb&w=600&h=400&fit=crop',
  ...
  const fetchTourPackages = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/tour-packages/moderator`, {
        params,
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      ...
      case 'published':
        return { icon: CheckCircle, color: 'bg-teal-100 text-teal-800' };
      case 'rejected':
        return { icon: XCircle, color: 'bg-red-100 text-red-800' };
      default:
        return { icon: AlertCircle, color: 'bg-gray-100 text-gray-800' };
    }
  };
      ...
      <div className="px-6 py-8 bg-gradient-to-r from-slate-900 to-blue-900">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col gap-2 max-w-2xl">
            <span className="text-sm font-medium tracking-wide text-teal-400">MODERATION DASHBOARD</span>
            <h1 className="text-3xl font-light text-white leading-tight">Tour Package Approvals</h1>
            <p className="text-slate-300">Review and manage tour packages submitted by guides</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 pb-12">
        <div className="mx-auto sm:px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              { status: 'pending', label: 'Pending Approval', count: stats.pending, icon: AlertCircle, color: 'bg-amber-100 text-amber-800' },
              { status: 'published', label: 'Published', count: stats.published, icon: CheckCircle, color: 'bg-teal-100 text-teal-800' },
              { status: 'rejected', label: 'Rejected', count: stats.rejected, icon: XCircle, color: 'bg-red-100 text-red-800' }
            ].map((stat) => (
              <div 
                key={stat.status}
                onClick={() => setSelectedTab(stat.status)}
                className={`p-6 rounded-xl shadow-sm cursor-pointer transition-all ${selectedTab === stat.status ? 'ring-2 ring-blue-500' : 'bg-white'}`}
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-500">{stat.label}</p>
                    <p className="text-3xl font-light mt-1 text-slate-800">{stat.count}</p>
                  </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div className="relative w-full md:w-96">
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-slate-700"
              />
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {tours.map((tour) => {
                const StatusIcon = getStatusInfo(tour.status).icon;
                const statusColor = getStatusInfo(tour.status).color;
                
                return (
                  <div key={tour.id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-all duration-300">
                    {/* Tour Image */}
                    <div className="relative h-48 w-full">
                      ...
                      <div className="absolute top-3 left-3">
                        <div className={`px-2 py-1 text-xs font-medium rounded-full flex items-center gap-1 ${statusColor}`}>
                    ...
                    <div className="p-5">
                      <div className="mb-3">
                        <h3 className="text-lg font-semibold text-slate-800 mb-1">{tour.title}</h3>
                      ...
                      <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
                        ...
                        {selectedTab === 'pending' && (
                          <>
                            <button
                              onClick={(e) => handleApprove(tour.id, e)}
                              className="flex-1 px-3 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500"
                            >
                              Approve
                            </button>
                            <button
                              onClick={(e) => handleReject(tour, e)}
                              className="flex-1 px-3 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                            >
                              Reject
                            </button>
                          </>
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={currentPage === 1}
                  className={`p-2 rounded-lg ${currentPage === 1 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-700 hover:bg-slate-100'}`}
                >
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg ${currentPage === page ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-slate-100'}`}
                  >
                    {page}
                  </button>
                ))}
                ...
                <button
                  onClick={() => handlePageChange(currentPage + 1)}
                  disabled={currentPage === totalPages}
                  className={`p-2 rounded-lg ${currentPage === totalPages ? 'text-slate-300 cursor-not-allowed' : 'text-slate-700 hover:bg-slate-100'}`}
                >
